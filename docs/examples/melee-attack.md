...
A complete melee attack ability that demonstrates the full GAS pipeline — effects, tags, abilities, input, montages, and the damage system working together. This builds on the concepts from [Getting Started](../getting-started/index.md) and shows a more complex ability with animation-driven hit timing and SetByCaller damage.

!!! tip "Prerequisites"
    This example assumes you've completed [Project Setup](../getting-started/project-setup.md) and have a working character with an ASC, AttributeSet (Health, Stamina, PendingDamage), and a base ability class. If you haven't, start there first.
...
- [Jump](jump.md) — the simplest GAS ability, great starting point
- [Dodge Roll](dodge-roll.md) — stamina cost, cooldown, i-frames, root motion
- [Ranged Attack](ranged-attack.md) — projectile spawning, target data, travel time
